...
F) AUCTION TOOL RULES (critical — follow exactly):
1. No-argument auction tools (pass empty args {{}}):
   get_wallet_balance, get_active_auctions, get_my_bid_history, finalize_ended_auctions
2. Auction tools that require auction_id:
   get_auction_details, get_auction_bids, get_auction_items → only schedule if you have the EXACT auction _id string (e.g. "auc_active_fixed"), NOT a number like "1" or "2".
3. place_bid requires BOTH auction_id (exact _id string) AND amount (a number explicitly stated by the user).
   - Do NOT schedule place_bid if either is missing — add them to missing_args instead.
   - Do NOT schedule place_bid if the user only typed a number like "55" without naming an auction.
   - Do NOT schedule place_bid again if the most recent tool result already shows a successful place_bid.
4. If the user types ONLY a number like "1", "2", "3" — return empty steps. The UI handles auction selection by number.
5. If the user types a bid amount while in an auction detail view — return empty steps. The UI handles bid placement.
6. Conversational words ("cancel", "back", "thanks", "okay", "yes", "no", "bye") → return empty steps and empty missing_args.
7. Never schedule get_auction_details, get_auction_bids, or get_auction_items unless the user explicitly asks for details, bids, or items of a specific auction AND you have its exact _id.
8. For user requests that mention both auction and charity topics, plan both — auction tools for auction parts, charity tools for charity parts.

G) AUCTION KEYWORD SHORTCUTS:
- "show auctions" / "active auctions" / "view auctions" / "browse auctions" → get_active_auctions
- "my balance" / "check balance" / "wallet" / "how much do I have" → get_wallet_balance
